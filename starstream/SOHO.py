...
class SOHO:
    ...
    class LASCO:
        url = (
            lambda self, date, name, tool: f"https://lasco-www.nrl.navy.mil/lz/level_05/{date[2:]}/{tool}/{name}"
        )
        url_img_txt = (
            lambda self, date, tool: f"https://lasco-www.nrl.navy.mil/lz/level_05/{date[2:]}/{tool}/img_hdr.txt"
        )
        lasco_root = "./data/SOHO/LASCO/"
        lasco_png_root = (
            lambda self, day, hour, tool: f"./data/SOHO/LASCO/{tool}/{day}_{hour}.png"
        )
        fits_root = (
            lambda self, day, hour, tool: f"./data/SOHO/LASCO/{tool}/{day}_{hour}.fits"
        )
        tools = ["c2", "c3"]
        ...
        def get_check_tasks(self, scrap_date: tuple[datetime, datetime]):
            ...
            self.new_scrap_date_list = [
                ...
                if glob.glob(self.lasco_png_root(date, "*", self.tool)) == []
            ]
        ...
        async def download_day(self, session, day):
            names_hours = await asyncio.gather(
                self.scrap_metadata(session, self.url_img_txt(day, self.tool))
            )
            await asyncio.gather(
                *[
                    self.download_url(session, name, tool, day, hour)
                    for tool, c in zip(self.tools, names_hours)
        ...
        async def download_url(self, name, tool, day, hour):
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    self.url(day, name, tool), ssl=False
                ) as response, aiofiles.open(self.fits_root(day, hour, tool)) as f:
        ...
        def get_slice(self, paths):
            return [
                cv2.resize(cv2.imread(path, cv2.IMREAD_GRAYSCALE), (512, 512))
                for path in paths
            ]
        ...
        def data_prep(self, scrap_date: tuple[datetime, datetime]):
            ...
            for date in datetime_interval(
                ...
                out.extend(
                    sorted(
                        glob.glob(f"./data/SOHO/LASCO/c2/{date}*.png"),
                        key=lambda name: name[-10:-4],
                    )
                )
            return out
