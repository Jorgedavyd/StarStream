...
class Hinode:
    class XRT(Img):
        def __init__(self, root: str = "./data/Hinode/XRT", batch_size: int = 1) -> None:
            super().__init__(root, batch_size, lambda name: osp.join(root, name), timedelta(hours = 1), '%Y%m%d-%H%M')
        ...
        def _interval_setup(self, scrap_date: ScrapDate) -> None:
            ...
            self.scrap_urls: List[str] = [self.url(*date.str().split('-'), "") for date in self.dates]
