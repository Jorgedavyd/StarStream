...
class OMNI(CDAWeb):
    url = lambda date: f'https://cdaweb.gsfc.nasa.gov/sp_phys/data/omni/hro2_5min/{date[0]}/omni_hro2_5min_{date[1]}01_v01.cdf'
    phy_obs = ['BX_GSE', 'BY_GSE', 'BZ_GSE', 'Mach_num', 'Mgs_mach_num', 'PR-FLX_10', 'PR-FLX_30', 'PR-FLX_60','proton_density', 'flow_speed', 'Vx', 'Vy', 'Vz']
    ...
    def check_tasks(self, scrap_date):
        ...
        self.new_scrap_date_list = [date for date in scrap_date if not os.path.exists(self.csv_file(date))]
    ...
    def get_data_tasks(self):
        data = [pd.read_csv(self.csv_file(date), parse_dates = ['datetime'], index_col = 'datetime')\
                for date in self.scrap_date]
        df = pd.concat(data, axis = 0).loc[pd.Timestamp(self.scrap_date[0]): pd.Timestamp(self.scrap_date[-1])]
        return df
