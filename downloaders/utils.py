...
def l1_hour_steps(init_hour: datetime, last_hour: datetime, seq_len: timedelta = timedelta(hours = 2)):
    init_hour = datetime.strptime(init_hour, '%Y%m%d') - seq_len
    return datetime.strftime(init_hour, '%Y%m%d-%H%M%S'), datetime.strftime(last_hour, '%Y%m%d-%H%M%S')
...
async def asyncZIP(bytes_obj: BytesIO, processing, *args):
    zip_file = await asyncio.get_event_loop().run_in_executor(None, zipfile.ZipFile, bytes_obj)
    if iscoroutinefunction(processing):
...
async def asyncFITS(bytes_obj: BytesIO, processing, *args):
    fits_file = await asyncio.get_event_loop().run_in_executor(None, fits.open, bytes_obj)
    if iscoroutinefunction(processing):
...
def flare_hour_steps(init_hour: datetime, last_hour: datetime, step_size, int_seq_len: int):
    assert (step_size>=timedelta(minutes = 2)), '2 minutes is the highest resolution'
    hours = []
    current_hour = init_hour
    while current_hour <= last_hour:
        hours.extend([current_hour - step*step_size for step in range(int_seq_len, -1, -1)]) 
...
def interval_time(start_date_str, end_date_str, format = "%Y%m%d", step_size: timedelta = timedelta(days = 1), output_format = None):
...
class DataDownloading:
    @staticmethod
    async def download(sat_objs: list, scrap_date: tuple[datetime,datetime]):
        async with aiohttp.ClientSession() as session:
            await asyncio.gather(*[sat_obj.downloader_pipeline(scrap_date, session) for sat_obj in sat_objs])