...
class CDAWeb:
    def default_cda_processing(self, cdf_file, date):
        data_columns = [cdf_file[var][:].reshape(-1, 1) if len(cdf_file[var][:].shape) == 1 else cdf_file[var][:] for var in self.phy_obs]
        data = np.concatenate(data_columns, axis=1).astype(np.float32)
        pd.DataFrame(data, columns = self.variables, index = pd.to_datetime(cdf_file['Epoch'][:].reshape(-1))).resample('1T').mean().to_csv(self.csv_path(date))
    ...
    def check_tasks(self, scrap_date: tuple[datetime, datetime]):
        ...
        self.new_scrap_date_list = [date for date in scrap_date if not os.path.exists(self.csv_path(date))]
    ...
    def data_prep(self, scrap_date: tuple[datetime, datetime], step_size: timedelta):
        ...
        return df[(df.index >= init) & (df.index<=end)].resample(timedelta_to_freq(step_size)).mean()
