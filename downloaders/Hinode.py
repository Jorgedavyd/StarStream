...
class Hinode:
    class XRT:
        ...
        download_urls = []
        url = lambda date, hour: f'https://xrt.cfa.harvard.edu/level1/{date[:4]}/{date[4:6]}/{date[6:]}/H{hour[:2]}00/'
        
        def __init__(self, sequence_length):
            self.sl = sequence_length

        def get_check_tasks(self, scrap_date):
            scrap_date = interval_time(scrap_date[0], scrap_date[-1], step_size = timedelta(hours = 1), output_format='%Y%m%d-%H%M%S')
            self.new_scrap_date_list = [date for date in scrap_date if not os.path.exists(self.xrt_npy_path(date))]

        def get_scrap_tasks(self, session):
            return [self.scrap_names(session, *date) for date in self.new_scrap_date_list]

        async def scrap_names(self, session, date, hour):
            async with session.get(self.url(date, hour)) as response:
                html = await response.text
                soup = BeautifulSoup(html, 'html.parser')
                fits_links = soup.find_all('a', href = lambda href: href and href.endswith('.fits'))
                self.download_urls.append([((date, hour),self.url(date, hour) + link['href']) for link in fits_links if link if timedelta(minutes = datetime.strptime(fits_links.split('_')[-1][:-7], '%H%M%S').minute)>timedelta(minutes = 45)])  
                
        def get_download_tasks(self, session):
            return [self.download_url(session, date, url) for date, url in self.download_urls]
        ...
        async def download_url(self, session, date, url):
            async with session.get(url) as response:
                ...
                await asyncFITS(BytesIO(data), self.fits_processing, self.xrt_npy_path(date))
