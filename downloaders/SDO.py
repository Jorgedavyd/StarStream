...
class SDO:
    class AIA_HR:
        ...
        batch_size = 10
        wavelengths: list = ['94', '131', '171', '193', '211','304', '335', '1600', '1700', '4500']
        def __init__(self, step_size: timedelta, wavelength: str|int):
            ...
            assert (step_size>self.min_step_size), 'Not valid step size, extremely high resolution'
            self.step_size = step_size
            self.wavelength = wavelength
            self.url = lambda date, name: f'http://jsoc2.stanford.edu/data/aia/images/{date[:4]}/{date[4:6]}/{date[6:]}/{wavelength}/{name}'
            ...
            self.name = lambda webname: '-'.join([item.replace('_','') for item in webname.split('__')[:-1]]) + '.jp2'
        ...
        def check_tasks(self, scrap_date: tuple[datetime, datetime]):
            ...
            self.new_scrap_date_list = [date for date in scrap_date if len(glob.glob(self.scrap_path(date)))<((timedelta(days = 1)/self.step_size)-1)]
        ...
        async def get_names(self, html):
            ...
            return [names[i] for i in range(0,len(names), self.step_size//self.min_step_size)]
        ...
        async def download_from_name(self, name):
            ...
            async with aiohttp.ClientSession() as client:
                async with client.get(url, ssl = False) as response, aiofiles.open(self.jp2_path(self.name(name)), 'wb') as f:
                    await f.write(await response.read())
        
        async def batched_download(self):
            params = []
            for i in tqdm(range(0, len(self.new_scrap_date_list), self.batch_size), desc = 'Getting file names...'):
                params.extend([*chain.from_iterable(await asyncio.gather(*[self.scrap_names(date) for date in self.new_scrap_date_list[i:i+self.batch_size]]))])
                
            for i in tqdm(range(0, len(params), self.batch_size), desc = 'Getting images...'):
                await asyncio.gather(*[self.download_from_name(name) for name in params[i: i+self.batch_size]])          

        def data_prep(self, scrap_date: tuple[datetime, datetime]):
            scrap_date = datetime_interval(*scrap_date, timedelta(days = 1))
            return [*chain.from_iterable([glob.glob(self.scrap_path(date)) for date in scrap_date])]
    ...
    class AIA_LR:
        ...
        def __init__(self, wavelength: str):
            self.wavelengths: list = ['0094', '0131', '0171', '0193', '0211','0304', '0335', '1600', '1700', '4500']
            assert (wavelength in self.wavelengths), f'Not valid wavelength: {self.wavelengths}'
            self.wavelength = wavelength
            self.url = lambda date, name: f'https://sdo.gsfc.nasa.gov/assets/img/browse/{date[:4]}/{date[4:6]}/{date[6:]}/{name}'
        ...
        def check_tasks(self, scrap_date: tuple[datetime, datetime]):
            scrap_date = datetime_interval(*scrap_date, timedelta(days = 1))
            self.new_scrap_date_list = [date for date in scrap_date if len(glob.glob(self.scrap_path(date)))==0]
        ...
        def find_all(self, soup):
            return soup.find_all('a', href = lambda href: href.endswith(f'512_{self.wavelength}.jpg'))

        async def download_from_name(self, name):
            date = name.split('_')[0]
            url = self.url(date, name)
            async with aiohttp.ClientSession() as client:
                async with client.get(url, ssl = False) as response, aiofiles.open(self.jpg_path(self.name(name)), 'wb') as f:
                    await f.write(await response.read())
    ...
    class EVE:
            batch_size = 10
            url = lambda self, date: f'https://lasp.colorado.edu/eve/data_access/eve_data/products/level1/esp/{date[:4]}/esp_L1_{date_to_day_of_year(date)}_007.fit.gz'
            ...
            async def to_csv(self, fits_file, day):
                df = await asyncio.get_event_loop().run_in_executor(None, pd.DataFrame, fits_file[1].data)
                df.index = await asyncio.get_event_loop().run_in_executor(None, df.apply,
                                    lambda row: datetime(int(row['YEAR']), 1, 1) + 
                                    timedelta(days=int(row['DOY']) - 1, seconds=int(row['SOD']), 
                                            microseconds=int((row['SOD'] - int(row['SOD'])) * 1e6)), 1)
                df[['CH_18', 'CH_26', 'CH_30','Q_1', 'Q_2', 'Q_3']].resample('1T').mean().to_csv(self.eve_csv_path(day))

            def get_check_tasks(self, scrap_date):
                scrap_date = datetime_interval(scrap_date[0], scrap_date[-1], timedelta(days = 1))
                self.new_scrap_date_list = [date for date in scrap_date if not os.path.exists(self.eve_fits_gz_path(date))]

            async def download_task(self, session):
                for i in range(0, len(self.new_scrap_date_list), self.batch_size):
                    await asyncio.gather(*[self.download_url(session, day) for day in self.new_scrap_date_list[i: i+self.batch_size]])
            ...
            def data_prep(self, scrap_date: tuple[datetime, datetime], step_size):
                ...
                return cudf.concat(dfs).resample(timedelta_to_freq(step_size)).mean().loc[init:end]      
