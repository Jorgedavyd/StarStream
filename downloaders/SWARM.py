...
def mag_ion_transform(df: cudf.DataFrame):
    ...
    df[['Vicrx', 'Vicry', 'Vicrz']] = nec_transform(df[['Vicrx', 'Vicry', 'Vicrz']].values)
...
class SwarmUtils:
    ...
    def check_tasks(self, scrap_date: tuple[datetime, datetime]):
        self.new_scrap_date_list = [datetime.strptime(date, '%Y%m%d') for date in datetime_interval(scrap_date[0], scrap_date[-1], timedelta(days = 1)) if not os.path.exists(self.csv_path(date, 'A'))]
    ...
    def data_prep(self, scrap_date: tuple[datetime, datetime], step_size: timedelta):
        ...
        dfs = [cudf.read_csv(self.csv_path(date, X)) for date in scrap_date for X in self.spacecrafts]
    ...
    def download_url(self, X, date_interval):
        ...
        for date in self.new_scrap_date_list:
            df[df.index.date == pd.Timestamp(date).date()].to_csv(self.csv_path(datetime.strftime(date, '%Y%m%d'), X))
...
class SWARM:
    ...
    class MAG_ION(SwarmUtils):
        ...
        measurements = ['Bx', 'By', 'Bz', 'Ehx', 'Ehy', 'Ehz', 'Evx', 'Evy', 'Evz', 'Vicrx', 'Vicry', 'Vicrz', 'Vixv', 'Vixh', 'Viy', 'Viz']
        def __init__(self):
            self.create_folders()
